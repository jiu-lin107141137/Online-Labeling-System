...
const w: Ref<number> = ref(210), h: Ref<number> = ref(100);
...
const generateCode = () => {
  ...
  for(let i = 0; i < 4; i++)
    code.value += chars.charAt(randomInt(0, chars.length));
  // draw background
  ctx.value.fillStyle = "rgb("+randomInt(0, 127)+", "+randomInt(0, 127)+", "+randomInt(0, 127)+")"
  ctx.value.fillRect(0, 0, w.value, h.value)
  // draw lines
  for(let i = 0; i < 10; i++){
    ctx.value.strokeStyle = "rgb("+randomInt(127, 255)+", "+randomInt(127, 255)+", "+randomInt(127, 255)+")"
    ctx.value.beginPath()
  ...
  // draw points
  for(let i = 0; i < 100; i++){
    ctx.value.fillStyle = "rgb("+randomInt(127, 255)+", "+randomInt(127, 255)+", "+randomInt(127, 255)+")"
    ctx.value.beginPath();
